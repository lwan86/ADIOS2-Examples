...
public:
    ...
    ~IO();
    void read(int step, const Settings &s, MPI_Comm comm);
...
private:
    ...
    // rank and step
    std::string MakeFilename(const std::string &inputfile,
                             const std::string &suffix, int rank = -1,
                             int step = -1)
    {
        std::string name;
        int ss = inputfile.size();
        if (rank == -1 && step == -1)
        {
            ...
            if ((ss > suffix.size()) &&
                inputfile.find(suffix) != ss - suffix.size())
            {
                name += suffix;
            }
        }
        else
        {
            ...
            if ((ss > suffix.size()) &&
                inputfile.find(suffix) == ss - suffix.size())
            {
                name = inputfile.substr(0, ss - suffix.size());
            }
            if (rank >= 0)
            {
